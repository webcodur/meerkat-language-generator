...
export default function DragHandle({
  ...
  const calculateHeights = useCallback(() => {
    ...
    rows.forEach((row, index) => {
      ...
      if (selectedRows.includes(index)) {
        ...
        blockTailBot = Math.max(
          blockTailBot,
          (row as HTMLElement).offsetTop + rect.height
        );
      }
    });
   ...
   */
  const getRowBoundaries = useCallback(
    (rowPositions: number[]): RowBoundary[] => {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (targetIndex !== null) {
        // 이전 미리보기 효과 제거
        document
          .querySelectorAll(".preview-move, .preview-move-invalid")
          .forEach((el) => {
        ...
        if (targetRow) {
          ...
          if (
            targetIndex >= minSelectedIndex &&
            targetIndex <= maxSelectedIndex + 1
          ) {
