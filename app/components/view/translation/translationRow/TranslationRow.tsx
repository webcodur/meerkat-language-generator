...
export default function TranslationRow({
	...
	return (
		<div data-row-id={index} className="flex items-center space-x-2 py-2 border-b min-h-[42px]">
			{/* 1. 체크박스 */}
			<CheckboxCell index={index} selectedRows={selectedRows} onRowSelect={onRowSelect} />

			{/* 2. 행 번호 */}
			<RowNumberCell index={index} />

			{/* 3. 한국어 단어 입력 */}
			<KoreanInputCell row={row} index={index} onUpdate={onUpdate} type="word" />

			{/* 4. 한국어 설명 입력 */}
			<KoreanInputCell row={row} index={index} onUpdate={onUpdate} type="description" />

			{/* 5. 번역 생성 버튼 */}
			<GenerateButton row={row} index={index} loadingRows={loadingRows} onSubmit={onSubmit} />
			...
			<TranslationInput
				...
				onChange={(value) => onUpdate(index, { ...row, englishTranslation: value })}
				disabled={row.isVerified}
				language="en-US"
			...
			<TranslationInput
				...
				onChange={(value) => onUpdate(index, { ...row, arabicTranslation: value })}
				disabled={row.isVerified}
				language="ar-SA"
				width={parseInt(COLUMN_WIDTHS.translation)}
				isRTL={true}
