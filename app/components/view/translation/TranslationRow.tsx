...
export default function TranslationRow({
	...
	return (
		<div className="flex items-center space-x-2 py-2 border-b min-h-[42px]">
			<div
			...
			<textarea
				...
				onChange={(e) =>
					onUpdate(index, { ...row, koreanWord: e.target.value })
				}
			...
			<textarea
				...
				onChange={(e) =>
					onUpdate(index, { ...row, koreanDescription: e.target.value })
				}
          ...
          ${
			...
			<input
				...
				onChange={(e) =>
					onUpdate(index, { ...row, englishTranslation: e.target.value })
				}
				disabled={row.isVerified}
			...
			<input
				...
				onChange={(e) =>
					onUpdate(index, { ...row, arabicTranslation: e.target.value })
				}
				disabled={row.isVerified}
				className={`${
					COLUMN_WIDTHS.translation
				} ${inputClassName} text-right dir-rtl ${
					row.isVerified ? 'bg-gray-100 cursor-not-allowed' : ''
				}`}
				dir="rtl"
			/>
			<div
				...
				<input
					...
					onChange={(e) =>
						onUpdate(index, { ...row, isVerified: e.target.checked })
					}
