...
export default function BottomActions({
  ...
  const handleSave = async () => {
    try {
      ...
      // 중복 키 체크
      const keys = rows.map((row) => row.koreanWord.trim());
      const duplicateKeys = keys.filter(
        (key, index) => keys.indexOf(key) !== index
      );
      if (duplicateKeys.length > 0) {
        alert(`중복된 키값이 있습니다: ${duplicateKeys.join(", ")}`);
        return;
      }
      ...
      rows.forEach((row) => {
        const key = row.koreanWord.trim();
        if (key) {
          arTranslations[key] = row.arabicTranslation.trim();
          koTranslations[key] = key;
          enTranslations[key] = row.englishTranslation.trim();
          verifiedStates[key] = row.isVerified;
          descriptions[key] = (row.koreanDescription || "").trim();
        }
      });
